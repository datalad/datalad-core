...
def test_get_gitconfig_items_from_env(monkeypatch):
    ...
    with monkeypatch.context() as m:
        ...
        with pytest.raises(
            ValueError,
            match='bogus count in GIT_CONFIG_COUNT',
        ):  # pragma: no cover (ACK that branch coverage is not complete)
            get_gitconfig_items_from_env()

    # bunch of std error conditions
    for env, excstr in (
        (
            {'GIT_CONFIG_COUNT': 1, 'GIT_CONFIG_KEY_0': 'section.name'},
            'missing config value',
        ),
        ({'GIT_CONFIG_COUNT': 1, 'GIT_CONFIG_VALUE_0': 'value'}, 'missing config key'),
        (
            {
                'GIT_CONFIG_COUNT': 1,
                'GIT_CONFIG_KEY_0': '',
        ...
        with monkeypatch.context() as m:
            ...
            with pytest.raises(ValueError, match=excstr):
                get_gitconfig_items_from_env()
...
def test_set_gitconfig_items_in_env(monkeypatch):
    for start, items, target in (
        ...
        # but it has limits
        ({'GIT_CONFIG_COUNT': 'nochance'}, {}, {'GIT_CONFIG_COUNT': 'nochance'}),
...
def test_get_set_gitconfig_env_roundtrip(monkeypatch):
    items = {'section.name': ('c', 'a', 'b'), 'space section.na me.so me': 'v al'}
    with monkeypatch.context() as m:
        env = {}
        m.setattr(utils, 'environ', env)
