...
class GitWorktreeItem:
    """Item in a Git worktree

    The ``tree_path`` property associates the item with a concrete worktree
    (location) on the filesystem. The item itself is identified by its
    path relative to the worktree root. Analog to Git, this path is reported
    ...
    # @property
    # def name(self) -> str:
    #    """Name of the item is its ``str`` path relative to the tree root"""
    ...
    def path(self) -> Path:
        ...
        The item's ``tree_path`` determines whether the path is relative or
        absolute.
        """
        return self.tree_path / self.relpath
...
def iter_gitworktree(
    ...
    *,
    ...
    :class:`GitWorktreeItem` are yielded. Their ``gitsha`` and ``gittype``
    ...
    subm_to_process: list[GitWorktreeItem] = []
...
def _report_items_from_submodules(
    subms: Iterable[GitWorktreeItem],
    *,
    recursion: str,
    untracked: str | None,
...
def _iter_gitworktree(
    ...
    *,
    ...
    pending_item: GitWorktreeItem | None = None
    ...
    for line in chain(git_ls_files(path, *lsfiles_args), [None]):
        ...
        item = None if line is None else _lsfiles_line2item(path, line)
...
def _lsfiles_line2item(path: Path, line: str) -> GitWorktreeItem:
    ...
    return GitWorktreeItem(
        ...
        relpath=PurePosixPath(items[1]),
        gitsha=props[1],
        gittype=git_mode_type_map[props[0]] if props[0] else None,
    )
