...
def test_paramprocessor_multiparam_failure():
    ...
    with pytest.raises(ParamErrors) as e:
        pp({'p1': 'ignored', 'p2': 'ignored'}, on_error='raise-at-end')
    assert len(e.value.errors) == len(pspecs)
    # smoke test
    assert repr(e.value.errors).startswith('mappingproxy(')
    assert set(e.value.errors.messages) == {'err1', 'err2'}
    assert set(e.value.errors.context_labels) == set(pspecs)
    exc_str = str(e.value)
...
def test_paramprocessor_paramsets():
    ...
    with pytest.raises(ParamErrors) as e:
        pp(fail_case, on_error='raise-early')
    ...
    with pytest.raises(ParamErrors) as e:
        pp(fail_case, on_error='raise-at-end')
...
def test_paramprocessor_tailor_for_dataset(gitrepo):
    ...
    # thing work again
    pp = ParamProcessor(proc_defaults={'dataset'}, **pp_kwargs)
    res = pp({'dataset': None, 'path': 'dummy'}, at_default={'dataset'})
