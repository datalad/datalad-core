...
from datalad_core.iter_collections.utils import GitTreeItemType
...
def test_iter_gitworktree(gitrepo):
    ...
    assert all(
        isinstance(i, GitWorktreeItem) and i.gitsha and i.gittype for i in tracked_items
    )
...
def test_lsfiles_line2item(tmp_path):
    # untracked
    assert _lsfiles_line2item(tmp_path, 'some/file.txt') == GitWorktreeItem(
        tree_path=tmp_path, relpath=PurePosixPath('some', 'file.txt')
    )
    # tracked
    mode = '100644'
