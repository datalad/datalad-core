...
def get_gitconfig_items_from_env() -> dict[str, str | tuple[str, ...]]:
    ...
    for k, v in (
        (_get_gitconfig_var_from_env(i, 'key'), _get_gitconfig_var_from_env(i, 'value'))
...
def _get_gitconfig_itemcount() -> int:
    try:
        ...
        msg = 'bogus count in GIT_CONFIG_COUNT'
...
def _get_gitconfig_var_from_env(nid: int, kind: str) -> str:
    ...
    if var is None:
        msg = f'missing config {kind} {envname}'
        raise ValueError(msg)
    if kind != 'key':
        return var
    if not var:
        msg = f'empty config key {envname}'
        raise ValueError(msg)
    if '.' not in var:
        msg = f'key {envname} does not contain a section: {var}'
        raise ValueError(msg)
    return var
