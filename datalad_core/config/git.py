...
class GitConfig(CachingSource):
    ...
    def _get_git_config_cmd(self) -> list[str]:
        """Return the git-config base command suitable for a particular config"""
    ...
    def _load(self) -> None:
        ...
        for k, v in dct.items():
            ...
            self.setall(
                k,
                tuple(ConfigurationItem(val) for val in vals),
            )
...
class DataladBranchConfig(LocalGitConfig):
    ...
    # TODO: reimplement is_writable to say False for bare repos
    def __init__(self, path: PathLike):
        super().__init__(path)
